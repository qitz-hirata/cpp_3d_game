...
![キービジュアル](file:///D:/DirectXGames/cpp_3d_game/key_visual.png)

本プロジェクトは、**DxLib**を使用したC++による3Dアクションゲームの教育用または基礎開発用のサンプルプロジェクトです。

以下にプロジェクトのフォルダ構成ツリー、各ファイルの役割、主要クラス、アーキテクチャ設計および動作フローについて整理します。

---

## 1. フォルダ構成ツリー

プロジェクト全体のディレクトリ構造とソースファイル・アセットの配置関係です（`External/DxLib` 内部のライブラリ群など、ビルド成果物や一部の冗長なファイルは省略表記にしています）。

```text
cpp_3d_game/ (ルートディレクトリ)
├── .editorconfig
├── .gitattributes
├── .gitignore
├── 01_00.md 〜 01_04.md (学習教材テキスト)
├── 2025DxLibSotuken.vcxproj (VSプロジェクト)
├── 2025DxLibSotuken.vcxproj.filters
├── 2025DxLibSotuken.vcxproj.user
├── CPPDXGamePrograming.sln (VSソリューション)
├── CLAUDE.md (開発ガイドライン)
├── README.md
├── main.cpp (メインエントリー)
├── GameObject.h
...
├── Vector3.h
├── Quaternion.h
├── Cube.h / Cube.cpp (3Dキューブオブジェクト)
├── Player.h / Player.cpp (プレイヤー制御)
├── Enemy.h / Enemy.cpp (敵キャラクター)
├── EnemyManager.h / EnemyManager.cpp (敵マネージャー)
├── Bullet.h / Bullet.cpp (弾丸)
├── BulletManager.h / BulletManager.cpp (弾丸マネージャー)
├── CollisionManager.h / CollisionManager.cpp (衝突判定マネージャー)
├── External/ (外部ライブラリ)
│   └── DxLib/ (DxLib SDK 本体)
│       ├── DxLib.h
│       └── [各種ライブラリ・ヘッダーファイル群]
